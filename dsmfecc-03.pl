...
my $product_name = "Tivoli Storage Manager for Enterprise Resource Planning : Data Protection for SAP for DB2";
my @result = "";

my $opt_prefix = "--";
my %defined_options = (
      "applicationentity"  => [255, "true", "<database name>",     "required", "Name of the database."],      
...
{
   print"\nConnecting to $user_options{'applicationentity'}... \n\n";
   my $cmd_str = 'db2 "call get_dbsize_info(?,?,?,-1)"';
   my $con_str = "db2 connect to $user_options{'applicationentity'}";               
   
   my @con_result = `$con_str`;    
...
{  
   ...
   {              
      ...
      if ($split_lines)
      {
         ...
         $capacity_counting{'item_size'} = floor($size/1024/1024);
         $split_lines = 0;         
      }      
   }
